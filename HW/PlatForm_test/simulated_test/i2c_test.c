...
test_i2c_open_bus()
{
  ...
  assert_true( myBus == 0); // Error - there is not other bus
}
...
test_i2c_write_read()
{
  ...
  assert_int_equal( 0,
      I2C_send( address_ok_a, slaveRegister, noBytesTlow, tlowBytes));
  assert_int_equal( 0,
      I2C_read( address_ok_a, slaveRegister, noBytesTlow, &response[1]));
